...
- 工作树隔离：`worktree_create / worktree_run / worktree_remove / worktree_events`
...
- 执行面（Execution Plane）：工具执行、后台任务、子代理、worktree 车道
...
2. 风险改动或并行开发优先走 worktree：避免主目录互相污染。
3. 长任务优先使用会话恢复与压缩：保证上下文可持续。
4. 多 Agent 协作尽量走协议化流程：减少“各做各的”漂移。

## 目录结构

- `agents/main.py`：统一 CLI 入口
- `agents/s12_worktree_task_isolation.py`：当前最终运行时核心
- `maggie/`：核心模块（llm、tools、tasks、team、autonomy、worktrees、session）
- `skills/`：按需加载的技能文档
- `docs/`：架构与说明文档
- `explain/`：实现过程说明（内部学习向）
