...
from maggie.tools import execute_tool, tools_with_worktree_system
from maggie.worktrees import WorktreeManager


# s12 在 s11 的自治团队基础上，再补一层目录隔离：任务板负责协调，worktree 负责真正的执行车道。
SYSTEM_SUFFIX = (
    ...
    'Use worktree_create, worktree_list, worktree_status, worktree_run, worktree_keep, worktree_remove, and worktree_events when parallel or risky changes should be isolated by directory. '
    'Treat the persistent task board as the control plane and worktrees as execution lanes. '
    'Bind tasks to worktrees when isolating implementation work, and decide whether to keep or remove each lane during closeout. '
...
def build_system_with_skills(workdir: Path, skill_loader: SkillLoader, worktree_manager: WorktreeManager) -> str:
    # system prompt 只保留 skill 摘要和 worktree 能力提示，完整正文按需加载。
    repo_root = worktree_manager.repo_root
    git_hint = 'Git worktree tools are available.' if worktree_manager.git_available else 'Git worktree tools currently have no repository support and will return errors.'
    return (
        f"{build_system_prompt(workdir)} {SYSTEM_SUFFIX}\n\n"
        f'Repo root for worktree isolation: {repo_root}\n'
        f'{git_hint}\n\n'
        'Skills available:\n'
...
def format_team_inbox(messages: list[dict[str, Any]]) -> str:
    # 把 lead inbox 里的新消息注入主对话，避免团队沟通脱离主上下文。
...
def agent_loop(
    ...
    # 主 agent 统一管理会话、技能、后台任务、团队协议、任务板和 worktree 车道。
    ...
    worktree_manager = WorktreeManager(settings.workdir, task_manager)
    skill_loader = SkillLoader(settings.workdir / 'skills')
    system = build_system_with_skills(settings.workdir, skill_loader, worktree_manager)
    ...
    while True:
        ...
        if estimate_tokens(messages) > TOKEN_THRESHOLD:
            ...
            messages[:] = auto_compact(
                ...
                session_store,
                session_id,
                focus='Preserve current state, open todos, persistent tasks, worktree lanes, background jobs, team messages, protocol requests, autonomous work, loaded skills, and delegated work.',
            )

        response = client.create_message(
            system=system,
            messages=messages,
            tools=tools_with_worktree_system(),
        )
        ...
        if manual_compact:
            ...
            messages[:] = auto_compact(
                ...
                session_store,
                session_id,
                focus=manual_focus or 'Preserve current state, todos, persistent tasks, active worktrees, background jobs, team messages, protocol requests, autonomous work, and unfinished work.',
            )
            session_store.save_messages(session_id, messages)
...
def main() -> None:
    ...
    initial_worktree_manager = WorktreeManager(Path.cwd(), TaskManager(Path.cwd()))
    ...
    print(f'Repo root for s12: {initial_worktree_manager.repo_root}')
    if not initial_worktree_manager.git_available:
