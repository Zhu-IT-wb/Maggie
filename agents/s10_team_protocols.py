...
from maggie.team import MessageBus, ProtocolRegistry, TeammateManager
from maggie.todo import TodoManager
from maggie.tools import execute_tool, tools_with_protocol_team_system


# s10 在 s09 的 team mailbox 基础上，再加入结构化协议：shutdown 和 plan approval 都通过 request_id 相关联。
SYSTEM_SUFFIX = (
...
# 一次性子 agent 仍然保留给隔离探索使用；shutdown 和 plan approval 只发生在 persistent teammates 身上。
...
def agent_loop(
    ...
    # 主 agent 统一管理会话、Todo、skills、后台任务、团队成员和协议请求。
    ...
    while True:
        ...
        if estimate_tokens(messages) > TOKEN_THRESHOLD:
            ...
            messages[:] = auto_compact(
                ...
                session_store,
                session_id,
                focus='Preserve current state, open todos, persistent tasks, background jobs, team messages, protocol requests, loaded skills, and delegated work.',
            )

        response = client.create_message(
            system=system,
            messages=messages,
            tools=tools_with_protocol_team_system(),
        )
        ...
        if manual_compact:
            ...
            messages[:] = auto_compact(
                ...
                session_store,
                session_id,
                focus=manual_focus or 'Preserve current state, todos, persistent tasks, background jobs, team messages, protocol requests, and unfinished work.',
            )
            session_store.save_messages(session_id, messages)
...
if __name__ == '__main__':
    ...
    teammate_manager = TeammateManager(settings, Path.cwd() / '.team', message_bus, protocol_registry)
    ...
    while True:
        try:
            query = input('\033[36mMaggie s10 >> \033[0m')
