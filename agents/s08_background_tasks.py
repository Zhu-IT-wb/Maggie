...
from maggie.tools import execute_tool, tools_with_background_task_system


# s08 在 s07 的长期任务和会话能力之上，再加入后台任务：模型可以先发起耗时命令，再继续做别的事。
SYSTEM_SUFFIX = (
...
# 子 agent 仍然是隔离执行模型，但也要遵守 Windows shell 约束。
...
def agent_loop(
    ...
    # 父 agent 统一管理会话、Todo、skills、子 agent、长期任务和后台任务通知。
    ...
    while True:
        # 每次调用模型前，先把后台任务完成通知注入对话，避免结果一直悬空。
        ...
        if estimate_tokens(messages) > TOKEN_THRESHOLD:
            ...
            messages[:] = auto_compact(
                ...
                session_store,
                session_id,
                focus='Preserve current state, open todos, persistent tasks, background jobs, loaded skills, and delegated work.',
            )

        response = client.create_message(
            system=system,
            messages=messages,
            tools=tools_with_background_task_system(),
        )
        ...
        # Todo 仍然只负责短期执行提醒，不与长期任务板或后台任务状态混用。
        ...
        if manual_compact:
            ...
            messages[:] = auto_compact(
                ...
                session_store,
                session_id,
                focus=manual_focus or 'Preserve current state, todos, persistent tasks, background jobs, and unfinished work.',
            )
            session_store.save_messages(session_id, messages)
...
if __name__ == '__main__':
    ...
    while True:
        try:
            query = input('\033[36mMaggie s08 >> \033[0m')
        ...
        reply = agent_loop(history, session_store, session_id, background_manager)
        print(reply)
        print()